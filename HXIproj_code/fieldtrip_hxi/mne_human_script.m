...
% skullstrip gives you a bare brain (gray matter, white matter, and CSF)
% brain gives you a brainmask, with different number indicating different
% tissue:brain, skull, or scalp.

% segmentation of the mri
load mrirs;
cfg           = [];
cfg.coordsys  = 'ctf';
cfg.output    = {'scalp', 'skull', 'brain', 'skullstrip'};
seg           = ft_volumesegment(cfg, mrirs);
save seg seg;

%% Preprocessing of the anatomical MRI: realign to Talairach. 
% Talairach coordinate system need you to explicitly specify 
% LPA (l,, left ear), 
% RPA (r, right ear), 
% Nasion (n, intersection of eyes and nose),
% Extra control point (z, I choose the top of head).
% This coordinate system can be accepted by the following softwares: 
% FreeSurfer and MNE Suite.
...
%% Preprocessing of the anatomical MRI: save to disk (do it on server)
% % The code has to be run in matlab in linux system, becuase .mgz file can 
% % only be accessed by linux.
% 
% load mri_tal;
% load seg;
% 
...
% cfg.filetype    = 'mgz'; % suffix of the file will be .mgh
% cfg.parameter   = 'anatomy';
% ft_volumewrite(cfg, mri_tal);
% 
% cfg.filename    = [outputDirFull, filesep, prefix, 'masked'];
