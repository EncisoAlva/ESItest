...
% load C:/MyProject/data/input/Liu_Ola/amarnath_resting.mat; % amarnath_resting
%load C:/MyProject/data/input/Liu_Ola/brandon-resting.mat; % data_Brain_Resting_2_brandon
load C:/MyProject/data/input/Liu_Ola/hemond_resting_hx.mat; % hemond_resting
...
prefix = 'hemond-resting';%'amarnath_resting'; %'waterpain1';%'coldpressor2','brandon-resting','hemond-resting'
suffix = '.bdf';
fileName = [prefix, suffix];
fullName = [pathName, filesep, fileName];
    
for i = start : 2 : 2%57%start+step-1 %57
    prestim = -i;
...
%      dataFC_LP = hx_update_trial_resting(dataFC_LP, amarnath_resting);
%    dataFC_LP = hx_update_trial_resting(dataFC_LP, data_Brain_Resting_2_brandon);
    dataFC_LP = hx_update_trial_resting(dataFC_LP, hemond_resting);
...
%     save pattern pattern;
    pattern = [pattern, mean(abs(sdDIFF.avg.pow(:,1+512:end-512)),2)];%when there is no averaging
    save pattern pattern;
